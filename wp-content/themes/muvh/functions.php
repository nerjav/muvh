...
//Funci�n para ocultar p�ginas de la administraci�n al resto de usuarios - Elige los IDs de p�gina que no se mostrar�n
function columna_id_entradas($columnas){
    $columnas['wps_post_id'] = __('ID');
    return $columnas;
}
function columna_personalizada_id($nombre_columna, $id){
    if($nombre_columna === 'wps_post_id'){
        echo $id;
    }
}
add_filter('manage_posts_columns', 'columna_id_entradas', 5);
add_action('manage_posts_custom_column', 'columna_personalizada_id', 5, 2);
add_filter('manage_pages_columns', 'columna_id_entradas', 5);
add_action('manage_pages_custom_column', 'columna_personalizada_id', 5, 2);
